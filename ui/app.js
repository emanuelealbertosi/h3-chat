...
const localModels=initLocalModels({api,getState:()=>state,notify:toast,onChange:async(model,removed)=>{
  ...
  if(model)for(const [key,cap] of [['chat_model','chat'],['create_model','create'],['edit_model','edit'],['diagram_model','create'],['music_model','music']])if(settingsDraft[key]===model.id&&!model.capabilities.includes(cap))settingsDraft[key]='';
  if(removed)for(const key of ['chat_model','create_model','edit_model','diagram_model','music_model'])if(settingsDraft[key]===removed)settingsDraft[key]='';
...
function localCard(model){
  const m=model;
  return `<article class="model-card" data-external-card="${m.id}"><div class="model-top"><h3>${esc(m.name)}</h3><div class="model-link-actions"><button type="button" class="btn small" data-model-parameters="${m.id}">Parametri del modello</button><button type="button" class="btn small" data-edit-external="${m.id}">Modifica collegamento</button><button type="button" class="text-button" data-remove-external="${m.id}">Scollega</button></div></div><p>${esc(m.description)}</p><div class="model-meta"><span class="badge">PERCORSO ESTERNO</span><span class="badge ${m.ready?'ready':'warning'}">${m.ready?'Disponibile':'Non disponibile'}</span>${m.vision?`<span class="badge ${m.vision.enabled?'ready':'warning'}">${m.vision.enabled?'Vision attiva':'Non vision'}</span>`:''}${m.thinking?.supported?'<span class="badge">THINK</span>':''}${m.mtp?.supported?'<span class="badge">MTP</span>':''}${m.capabilities.map(c=>`<span class="badge">${({chat:'CHAT',create:'CREA',edit:'MODIFICA',music:'MUSICA'})[c]||esc(c)}</span>`).join('')}</div>${m.external_problems?.length?`<p class="local-error">${esc(m.external_problems.join(' '))}</p>`:''}${m.vision?.warning?`<p class="small-note">${esc(m.vision.warning)}</p>`:''}<details><summary>Percorsi collegati e componenti</summary>${m.files.map(f=>`<div class="external-path"><strong>${esc(state.model_role_labels[f.role]||f.role)}</strong><span>${esc(f.path)}</span><small>${gb(f.size)}</small></div>`).join('')}${m.vision?.projector&&!m.files.some(f=>f.role==='mmproj')?`<div class="external-path"><strong>mmproj automatico</strong><span>${esc(m.vision.projector)}</span></div>`:''}<p class="small-note">${esc(m.license)}</p></details></article>`;
}
...
async function renderChat(){
  ...
  for(const message of chat.messages){
    ...
    if(pending){
      const title=({create:'Creazione immagine',edit:'Modifica immagine',music:'Generazione musica',chat:message.meta.canvas?'Scrittura nel canvas':'Risposta in corso'})[message.meta.intent]||(job?.status==='queued'?'In attesa':'Preparazione della risposta');
    ...
    appendMedia(content,message.media);appendMusicDetails(content,message,state.settings.chat_advanced);
...
async function updateAssessment(){
  ...
  try{
    ...
    const roles={chat_model:'Chat / vision',create_model:'Creazione immagini',edit_model:'Modifica immagini',diagram_model:'Grafici e diagrammi',music_model:'Musica'};
