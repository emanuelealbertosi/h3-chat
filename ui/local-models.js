...
export function initLocalModels({api,getState,onChange,notify}){
  ...
  function renderFiles(){
    ...
    $('#local-model-files').innerHTML=profile.required.map(r=>pathField(r,true)).join('')+(draft.profile==='chat'?
      ...
      profile.optional.map(r=>pathField(r,false)).join('')+`<p class="small-note">Il profilo imposta i parametri predefiniti. Puoi modificarli in Preferenze → Impostazioni immagini → Avanzate.</p>${draft.profile.startsWith('anima')?'<p class="small-note">Encoder richiesto: Qwen3-0.6B Base. VAE: Qwen Image. Questo profilo genera immagini da testo.</p>':''}`);
