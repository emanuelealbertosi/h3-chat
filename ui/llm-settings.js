...
const pick=(draft,state)=>Object.fromEntries(state.llm_options.keys.map(k=>[k,draft[k]]));
export function syncLlmDraft(draft,state,next=draft.chat_model){
 ...
 if(next!==previous){draft.chat_model=next;Object.assign(draft,state.llm_options.defaults[draft.profile],draft.llm_overrides[next]||{});}
}
export function renderLlmPreferences(container,draft,state,onChange){
 ...
 const values=selected===draft.chat_model?pick(draft,state):{...state.llm_options.defaults[draft.profile],...draft.llm_overrides?.[selected]};
 ...
 container.querySelector('#llm-reset').onclick=()=>{Object.assign(values,state.llm_options.defaults[draft.profile]);persist();renderLlmPreferences(container,draft,state,onChange);};
 details();
}
