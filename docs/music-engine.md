...
La build del pacchetto Windows in CI usa `ci-install-music.py`: recupera il worker CPU della release con autenticazione anche per le bozze, verifica l’hash fissato e avvia il protocollo del binario. La compilazione da sorgenti resta riproducibile con lo script sopra; l’archivio distribuito contiene il motore effettivamente collaudato, non una ricompilazione diversa.
